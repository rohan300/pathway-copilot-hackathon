...
- A result the letter says is still AWAITED is not a completed step, however much of it has already come back. "AFB negative at four weeks, TB PCR negative, full culture still awaited" reports a partial result on a test that is STILL OUTSTANDING: leave report_date null and use status "ordered" or "booked", not done or reported. The same goes for "pending", "outstanding", "to follow", "we await" and a plan line saying to chase it. Only what the letter says has actually resulted is reported.
...
- A condition the letter attaches to a decision is a follow-up too: "clear for filgotinib if the consolidation has resolved and he remains TB culture negative at six weeks" promises the culture result at six weeks. Emit it with the item named as the letter names it ("full TB culture"), the interval in "phrase" ("at six weeks"), and "from" set to the procedure the interval counts from ("bronchoscopy") when the letter says so.
